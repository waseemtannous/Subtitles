...
This project aims to automate the process of transcribing Hebrew videos, translating the transcriptions into multiple languages, and adding subtitles to the videos. The main objective is to make video content accessible to a broader audience by providing subtitles in various languages. The pipeline will include the following steps:
...
Subtitles are created and added to the video using FFMPEG. For right-to-left languages like Hebrew and Arabic, the subtitles are formatted correctly to ensure proper display.
...
3. **Subtitle**: Generate subtitle files and overlay them onto the video using ffmpeg.
...
- **Single Language Processing**: The first version of the script supported only one target language at a time.
...
- **SRT File Type**: This format is the most widely used for subtitles. It is simple and easy to work with. The script generates an SRT file for each language enabling the user to easily upload the subtitles to video platforms. without needing to burn the subtitles to the video.
...
- **Modular Design**: The script is modular and easy to extend. Everything is wrapped in self-contained functions that can be easily imported and used in other projects.

- **Dynamic Environment Variables**: The script uses environment variables to configure the pipeline. The user can easily change the settings by modifying the `.env` file. This makes the script flexible and easy to use.

## Alternative Approaches

### Transcription Models

- **Other Transcription Models**: Experimented with various transcription models, but Whisper provided the best accuracy and integration ease. Its an offline model and does not require an internet connection.

- **Online Transcription Services**: Google Cloud Speech-to-Text and Microsoft Azure Speech-to-Text are popular online services. They also provide high accuracy but a cost is associated with using them.
...
- The whisper model works best when the audio is clear. The model might struggle with noisy audio or multiple speakers. It is recommended to use clean audio for best results.

- The whisper model yields good accuracy for text, punctuation, and timing. It is trained on a large dataset and can handle various accents and languages. Keep in mind that these models are not perfect and might make mistakes. During the testing, it missed only a handful of words in a 16 minutes video.
